...
func (cli *ClipRsClient) EncodeText(ctx context.Context, texts []string) ([][]float32, error) {
	...
	response, err := cli.client.EncodeText(ctx, request, grpc.MaxCallSendMsgSize(10000000))
	if err != nil {
		return nil, err
	}
...
func (cli *ClipRsClient) EncodeImage(ctx context.Context, images [][]byte) ([][]float32, error) {
	...
	response, err := cli.client.EncodeImage(ctx, request, grpc.MaxCallSendMsgSize(10000000))
	if err != nil {
		return nil, err
	}
	batch := make([][]float32, len(response.GetEmbedding()))
